...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
    ...
    /**
     * 截取字符串（目前包含表情，汉字，英文，数字，标点符号）
     * 字节数校验
     * @param str    目标字符串
     * @param length 截取长度
     * @return
     */
    private static final String REGEX_EMOJ_AND_CHAR = "[a-zA-Z]+|\\d\\s\\w+|-|\\[|\\]|[\\pP\\pZ\\pN]|[\\u4e00-\\u9fa5]|[\\ud83c\\udc00-\\ud83c\\udfff]|[\\ud83d\\udc00-\\ud83d\\udfff]|[\\u2600-\\u27ff]+";

    public static String ellipsisNoPoint(String str, int length) {
        if (str == null) {
            return "";
        }
        try {
            StringBuilder sb = new StringBuilder();
            int currentLength = 0;
            Pattern p = Pattern.compile(REGEX_EMOJ_AND_CHAR);
            Matcher m = p.matcher(str);
            while ( m.find() ) {
                currentLength += String.valueOf(m.group()).getBytes("UTF-8").length;
...
//                int m = arr[i]^arr[j];
//                System.out.println("比较"+ arr[i] + "," + arr[j] + " 结果 " + m);
//            }
//        }
//        String str = "晴天\uD83C\uDF39\"!/";
//        System.out.println( ellipsisNoPoint(str,13));
//    }
}
